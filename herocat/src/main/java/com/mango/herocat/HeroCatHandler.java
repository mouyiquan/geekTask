...
public class HeroCatHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof HttpRequest) {
            ...
            if (uri.contains(HeroCatServer.projectName)){
                uri = uri.replace("/" + HeroCatServer.projectName,"");
            }else {
                //返回404
            ...
            if (nameToServletMap.containsKey(servletName)) {
                ...
                // double-check，双重检测锁：锁前加一次是不必每次来都加锁，锁后加一次是为了防止多个线程同时进入锁前的判断，然后都进入锁内，这样就会创建多个实例
            ...
            }else if(request.method().name().equalsIgnoreCase("PUT")) {
                servlet.doPut(req, res);
            }else if(request.method().name().equalsIgnoreCase("DELETE")) {
